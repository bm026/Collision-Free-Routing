...
{ 
  ...
  prints("nodes: "); printn(nodenum); newline();
...
{ if (symbol = s_skip)
  ...
  then
  { nextsymbol();
    return cons2(s_network, rstatement())
  }
  else
  if (symbol = s_begin)
  then
  ...
  if (symbol = s_name) or (symbol = s_number)
  then
  { a := relement();
...
{ a := rstatement();
  ...
  then
  { nextsymbol();
    return cons3(s_and, a, rstatements())
  }
  else 
    return a
}
...
{ op := tree[x + t_op];
  ...
  if (op = s_semicolon) or (symbol = s_and)
  then
...
{ nodenum := 0;
  ...
  while (~lastnode) and (nodenum > 0) do
    translate1(t)
}
  

proc translate1(val t) is
  var s;
  var dlab;
  var mainlab;
...
{ op := tree[x + t_op];
  if (op = s_network)
  then 
    genstatement(findnode(tree[x + t_op1]), seq, clab, tail)
  else
  if (op = s_semicolon)
  then
  ...
  { if (op = s_skip) 
    ...
    if (op = s_input)
    then
      geninput(tree[x + t_op1], tree[x + t_op2])
    else
    if (op = s_output)
    then
      genoutput(tree[x + t_op1], tree[x + t_op2])
    else
    if op = s_return 
    then 
...
proc geninput(val ch, val right) is
  var sp;
  var rightop;
  var name;
  var base;
...
{ rightop := tree[right + t_op];
  ...
  { base := tree[right + t_op1];
    offset := tree[right + t_op2];
    if isval(offset)
    then
    { value := getval(offset);
      texp(ch);
...
proc genoutput(val ch, val x) is
  var sp;
{ if regsfor(ch) < 2
  then
  { texp(x);
...
proc outhdr() is
  var length;
{ length := cb_loadpoint + ((constp+stringp) << 2);
  outword(length);
  outword(constp + stringp)
}
